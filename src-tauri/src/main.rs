...
use psychopass_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m pytauri_ap`,
    // i.e, run the `src-tauri/python/pytauri_ap/__main__.py`
    let py_script = PythonScript::Module("psychopass".into());
